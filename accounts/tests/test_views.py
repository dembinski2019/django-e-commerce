...
class RegisterViewTestCase(TestCase):
    ...
    def test_register_ok(self):
        data = {
            'username':'everton','password1':'dembinski123',
            'password2':'dembinski123','email':'teste@test.com'
            }
    ...
    def test_register_error(self):
        data = {'username':'everton','password1':'dembinski123','password2':'dembinski123'}
        response = self.client.post(self.register_url, data)
...
class UpdatePasswordTestCase(TestCase):
    ...
    def test_update_password_ok(self):
        data = {'old_password':'123','new_password1':'dembinski123','new_password2':'dembinski123'}
        self.client.login(username=self.user.username, password='123')
        response = self.client.post(self.url, data)
        self.user.refresh_from_db()
        self.assertTrue(self.user.check_password('dembinski123'))
